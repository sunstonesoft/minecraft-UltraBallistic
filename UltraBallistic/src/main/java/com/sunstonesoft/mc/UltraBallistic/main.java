...
public final class main extends JavaPlugin {
	...
	public Boolean launchRocket(Location startloc, World world, Player player, int id) {
		...
		int x = (int) getConfig().get(String.format("inflight.%s.loc.x", id));
		int y = (int) getConfig().get(String.format("inflight.%s.loc.y", id));
		int z = (int) getConfig().get(String.format("inflight.%s.loc.z", id));
		Location loc = new Location(world, x, y ,z);
		Location landLoc = new Location(world, x, world.getHighestBlockYAt(startloc) ,z);
		
		world.spawnParticle(Particle.SMOKE_NORMAL, loc, 15);
		
		if (loc.getY() <= landLoc.getY()) {
...
//				}
		...
		if (cmd.getName().equalsIgnoreCase("launch_rocket")) { // If the player typed /basic then do the following, note: If you only registered this executor for one command, you don't need this
			if (!(sender instanceof Player)) {
				...
				getConfig().set(String.format("inflight.%s.id", id), id);
				getConfig().set(String.format("inflight.%s.loc.x", id), x);
				getConfig().set(String.format("inflight.%s.loc.y", id), y);
				getConfig().set(String.format("inflight.%s.loc.z", id), z);
				saveConfig();
				...
				new BukkitRunnable() {
				    ...
				    public void run() {
						...
						if (ret == true) {
							getConfig().set(String.format("inflight.%s", id), null);
							cancel();
						}
				    }
